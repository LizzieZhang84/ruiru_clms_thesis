...
def process_files(path1, path2, output_file):
    # List all Excel files in the first directory
    files1 = [f for f in os.listdir(path1) if f.endswith('.xlsx')]
    files2 = [f for f in os.listdir(path2) if f.endswith('.xlsx')]
    
    # Match files based on the base name (excluding extension)
    files1_set = set(os.path.splitext(f)[0] for f in files1)
    files2_set = set(os.path.splitext(f)[0] for f in files2)
    
    common_files = files1_set.intersection(files2_set)
    
    for file_base in common_files:
        file1 = os.path.join(path1, file_base + '.xlsx')
        file2 = os.path.join(path2, file_base + '.xlsx')
        data_idx = file_base
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python compare_annotations.py <path1> <path2> <output_file.xlsx>")
    else:
        path1 = sys.argv[1]
        path2 = sys.argv[2]
        output_file = sys.argv[3]
        process_files(path1, path2, output_file)
